	...
	function getFreeTextTemplate(rhc_id,rhc_element,rhc_elementdesc) {
        rhc_element = rhc_element.replace(/\"/g,'\'');
		...
		html += '<textarea style="width:600px;height:100px" name="rhc_elementdesc'+rhc_id+'" id="rhc_elementdesc'+rhc_id+'">' + rhc_elementdesc.replace(/&/gi,"&amp;") + '</textarea></div>';
	...
	function art_publish(future_publish) {
		...
		if(art_validate(1)) {
			...
			if(document.formArticle.hdn_is_editor.value == 1 || document.formArticle.hdn_is_admin.value == 1) {
				...
				// validate date range
				// changed to allow for back dating of the article
				// Tracker ticket Feature #303  -- there is no longer a date range check
				/*
	...
	function art_window_close() {
		...
		if($("#article_return_page").length>0){
			...
			window.location = base_url_ajax+"rfadmin/articles/list_articles.php?page=1";
		}
		//}
	}
	...
	function art_unique_nicename_chk(flag) {
		...
		if($('#txtnicename').val() != $('#hdnoldnicename').val()) {
			...
			$.post(base_url_ajax+"rfadmin/articles/ajax_controller.php?f=nicenamecheck&rnd="+Math.random(), {
	...
	function art_generate_nicename() {
		...
		if($('#hdn_ref_article_id').val() == "" && $('#txttitle').val() != "") {
			...
			$.post(base_url_ajax+"rfadmin/articles/ajax_controller.php?f=nicenamesuggest&rnd="+Math.random(), {
	...
	function art_template_id(default_value, selected_value){
		...
		$.getJSON('/rfadmin/articles/ajax_controller.php?f=art_template_get_id&p1='+template_type, function(jdata) {
	...
	function art_select_pre_template(){
		...
		$.get('/rfadmin/articles/ajax_controller.php?f=art_pre_template_data&p1='+pre_template_id, function(data) {
			if (data == "") {
				return false;
			}
