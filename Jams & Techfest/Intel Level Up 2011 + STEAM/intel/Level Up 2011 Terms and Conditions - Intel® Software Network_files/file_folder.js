$(function(){
	if($('#fileToUpload').length || $('#editorToggle').length || $('.file-upload-button').length) {
		init_file_upload();
	}
...
function fileupload(folder_id) {
	...
	function set_download(email_required,download_id,path,extra){
		...
		act_download(download_id,path,email,download_path,extra);
	}
	function act_download(download_id,path,email,download_path,extra){
	...
	function submit_sitecatalyst(path) {
		var path = path;
		var path_split = path.split("/");
                var course_download= "course_materials-"+path_split[9]+".zip";
                submit_track_download(path,course_download);
                }
	...
	function track_download(url,email,file_name){
		...
		if(ref.length > 96){    
                ...
                + "&wa_eCustom22=" + email
                + "&wa_events=se_cust22";
				try { waTrackAsLink(file_name,"d", wa_customStuff,"N");	} 
				catch (err) {}
        }

	function submit_track_download(url,coursename) {
                var ref = document.referrer;
                if(url.length > 96){
                        url = url.substring(0,96);
                }
                ...
                + "&wa_custom09=" + url
                + "&wa_events=se_cust22";
                                try { waTrackAsLink(coursename,"d", wa_customStuff,"N"); }
                                catch (err) {}
